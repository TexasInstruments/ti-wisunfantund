 ...
 *  ======== clk0Fxn =======
 */
void clk0Fxn(const boost::system::error_code& ec)
{
    if(arm_hal_callback)
    {
        arm_hal_callback();
...
void timerCallback(const boost::system::error_code& ec) {
    tr_info("Timer callback fired at %lu!", get_time_in_us());
}
...
{
    // int duration = 50; // microseconds
    boost::asio::deadline_timer timer(io_service, boost::posix_time::milliseconds(slots));
    // std::unique_lock<std::mutex> lk(cv_m);
    timer.async_wait(clk0Fxn);
    cv.notify_all();
