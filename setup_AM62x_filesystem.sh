...
AM62X_BOOT_SETUP_FOLDER=/am62x-setup/boot-setup
# AM64 directories
...
### USAGE ###
USAGE="USAGE: ./setup_AM62x_filesystem.sh <AM62X_SDK_INSTALL_PATH> [<LIB_COAP_INSTALL_PATH>]"
USAGE_EG="EXAMPLE: ./setup_AM62x_filesystem.sh /home/ti/ti-processor-sdk-linux-am62xx-evm-11.00.09.04/ /home/libcoap-release-4.3.0/"
USAGE_REQ="Requirements:\n1. Ensure to have setup the target filesys folder at <AM62X_SDK_INSTALL_PATH>/targetNFS following setup scripts in AM62x SDK. 
\n2. To cross compile libcoap. Download libcoap from https://github.com/obgm/libcoap/tree/release-4.3.0 & specify its path
\n3. Recommended AM62x SDK version = 11.0.x"
...
case "$1" in
*/)
	AM62X_ENV_PATH=$1$AM62X_ENV
	AM62X_FILESYS_PATH=$1$AM62X_FILESYS_REL_PATH
;;
*)
	AM62X_ENV_PATH=$1/$AM62X_ENV
	AM62X_FILESYS_PATH=$1/$AM62X_FILESYS_REL_PATH
...
WFANTUND_PATH=$AM62X_FILESYS_PATH$WFANTUND_INSTALL_PATH

### Echo Paths ###
echo "************* Directory paths *************"
echo "Wfantund base directory path"
echo $BASE_PATH
echo "\nAM62x environment file path"
echo $AM62X_ENV_PATH
echo "\nAM62x targetfilesys relative path"
echo $AM62X_FILESYS_PATH
...
if [ ! \( -f $AM62X_ENV_PATH \) ]
then
	echo "\nError: ENV PATH file does not exist? Expected relative env filepath: <SDK_PATH>/$AM62X_ENV"
	echo $USAGE
	echo $USAGE_EG
	echo $USAGE_REQ
	exit 0
...
if [ ! \( -d $AM62X_FILESYS_PATH \) ]
then
	echo "\nError: AM62x default filesystem folder note created. Expected filesystem folder to be at: $AM62X_FILESYS_PATH"
	echo $USAGE
	echo $USAGE_EG
	echo $USAGE_REQ
	exit 0
...
then
	...
	# Source the environment config required for AM62x
	. $AM62X_ENV_PATH
	# Run the bootstap.sh
	sudo ./bootstrap.sh
...
echo "\n************* Setting up Node.js v16.13.2 on AM62x *************"
...
# Install Node.js to the AM62x Target Filesystem
echo "Installing Node.js v16.13.2 to <TargetNFS>$NODEJS_INSTALL_PATH..."
cd $AM62X_FILESYS_PATH$NODEJS_INSTALL_PATH
...
echo "\n************* Setting up Webserver and WebApp Service on AM62x *************"
...
sudo npm install
sudo npm run build
# Copy built webapp folder to am62x
sudo cp -r $BASE_PATH$WEBAPP_PATH $AM62X_FILESYS_PATH$WEBAPP_INSTALL_PATH
# Make start script executable and copy to am62x
sudo chmod +x $BASE_PATH$AM62X_BOOT_SETUP_FOLDER/start-ti-wisun-webapp.sh
sudo cp $BASE_PATH$AM62X_BOOT_SETUP_FOLDER/start-ti-wisun-webapp.sh $AM62X_FILESYS_PATH$WEBAPP_INSTALL_PATH
# Set executable and cp init-script.sh to am62x init.d
INIT_SCRIPT_PATH=$BASE_PATH$AM62X_BOOT_SETUP_FOLDER/init-script.sh
INIT_SCRIPT_INSTALL_PATH=$AM62X_FILESYS_PATH/etc/init.d/init-script.sh
...
sudo ln -s ../init.d/init-script.sh $AM62X_FILESYS_PATH/etc/rc0.d/K20init-script.sh
sudo ln -s ../init.d/init-script.sh $AM62X_FILESYS_PATH/etc/rc1.d/K20init-script.sh
sudo ln -s ../init.d/init-script.sh $AM62X_FILESYS_PATH/etc/rc2.d/S20init-script.sh
sudo ln -s ../init.d/init-script.sh $AM62X_FILESYS_PATH/etc/rc3.d/S20init-script.sh
sudo ln -s ../init.d/init-script.sh $AM62X_FILESYS_PATH/etc/rc4.d/S20init-script.sh
sudo ln -s ../init.d/init-script.sh $AM62X_FILESYS_PATH/etc/rc5.d/S20init-script.sh
sudo ln -s ../init.d/init-script.sh $AM62X_FILESYS_PATH/etc/rc6.d/K20init-script.sh
...
sudo ./autogen.sh
# Configure lib coap 
./configure --host=aarch64-linux  --target=aarch64-linux --prefix=$AM62X_FILESYS_PATH -disable-doxygen --disable-manpages
...
COAP_CLIENT_PATH=$AM62X_FILESYS_PATH$COAP_CLIENT_INSTALL_PATH
