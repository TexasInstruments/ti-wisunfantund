...
DBusIPCAPI::DBusIPCAPI(DBusConnection *connection)
...
DBusIPCAPI::~DBusIPCAPI()
{
	dbus_connection_unref(mConnection);
}

void
DBusIPCAPI::init_callback_tables()
{
#define INTERFACE_CALLBACK_CONNECT(cmd_name, member_func) \
	mInterfaceCallbackTable[(cmd_name)] = boost::bind( \
		&DBusIPCAPI::member_func, this, _1, _2)

	INTERFACE_CALLBACK_CONNECT(WPAN_CMD_RESET_NCP, reset_NCP);
...
{
	delete (std::pair<NCPControlInterface*, DBusIPCAPI*> *)user_data;
}

std::string
DBusIPCAPI::path_for_iface(NCPControlInterface* interface)
{
	return std::string(WPAN_TUNNEL_DBUS_PATH) + "/" + interface->get_name();
}

int
DBusIPCAPI::add_interface(NCPControlInterface* interface)
{
	static const DBusObjectPathVTable ipc_interface_vtable = {
		&ObjectPathUnregisterFunction_cb,
		&DBusIPCAPI::dbus_message_handler,
	};

	std::string name = interface->get_name();
	std::string path = std::string(WPAN_TUNNEL_DBUS_PATH) + "/" + name;

	std::pair<NCPControlInterface*, DBusIPCAPI*> *cb_data =
	    new std::pair<NCPControlInterface*, DBusIPCAPI*>(interface, this);
	...
	interface->mOnEnergyScanResult.connect(
		boost::bind(
			&DBusIPCAPI::received_energy_scan_result,
			this,
			interface,
			_1
		)
	...
	interface->mOnNetworkTimeUpdate.connect(
		boost::bind(
			&DBusIPCAPI::received_network_time_update,
			this,
			interface,
			_1
		)
...
void
DBusIPCAPI::CallbackWithStatus_Helper(int ret, DBusMessage *original_message)
{
...
void
DBusIPCAPI::received_beacon(NCPControlInterface* interface, const WPAN::NetworkInstance& network)
{
...
void
DBusIPCAPI::received_energy_scan_result(NCPControlInterface* interface,
...
void
DBusIPCAPI::property_changed(NCPControlInterface* interface,const std::string& key, const boost::any& value)
{
	DBusMessageIter iter;
	DBusMessage* signal;
	...
	syslog(LOG_DEBUG, "DBusAPI:PropChanged: %s - value: %s", path.c_str(), any_to_string(value).c_str());
...
void
DBusIPCAPI::status_response_helper(
...
void
DBusIPCAPI::CallbackWithStatusArg1_Helper(
...
DBusHandlerResult
DBusIPCAPI::interface_reset_handler(
	...
	interface->reset(boost::bind(&DBusIPCAPI::CallbackWithStatus_Helper,
								 this, _1, message));
...
DBusHandlerResult
DBusIPCAPI::interface_status_handler(
	NCPControlInterface* interface,
	DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
	...
	if (ncp_state_is_sleeping(ncp_state)
	 ...
	 || (ncp_state == UNINITIALIZED)
		...
		interface->refresh_state(boost::bind(&DBusIPCAPI::
...
DBusHandlerResult
DBusIPCAPI::interface_join_handler(
	NCPControlInterface* interface,
	DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
	...
	interface->join(
		options,
		boost::bind(
			&DBusIPCAPI::CallbackWithStatus_Helper,
			this,
			_1,
			message
		)
...
DBusHandlerResult
DBusIPCAPI::interface_form_handler(
	NCPControlInterface* interface,
	DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_leave_handler(
	NCPControlInterface* interface,
	DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
	...
	interface->leave(boost::bind(&DBusIPCAPI::CallbackWithStatus_Helper,
								 this, _1, message));
	ret = DBUS_HANDLER_RESULT_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_attach_handler(
	NCPControlInterface* interface,
	DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
	...
	interface->attach(boost::bind(&DBusIPCAPI::CallbackWithStatus_Helper,
								  this, _1, message));
	ret = DBUS_HANDLER_RESULT_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_begin_low_power_handler(
	NCPControlInterface* interface,
	DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
	...
	interface->begin_low_power(boost::bind(&DBusIPCAPI::CallbackWithStatus_Helper,
								 this, _1, message));
	ret = DBUS_HANDLER_RESULT_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_host_did_wake_handler(
	NCPControlInterface* interface,
	DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
	...
	interface->host_did_wake(boost::bind(&DBusIPCAPI::CallbackWithStatus_Helper,
								 this, _1, message));
	ret = DBUS_HANDLER_RESULT_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_net_scan_stop_handler(
	NCPControlInterface* interface,
	DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
	...
	interface->netscan_stop(boost::bind(&DBusIPCAPI::CallbackWithStatus_Helper,
									 this, _1, message));
	ret = DBUS_HANDLER_RESULT_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_discover_scan_stop_handler(
	NCPControlInterface* interface,
	DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_energy_scan_stop_handler(
	NCPControlInterface* interface,
	DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
	...
	interface->energyscan_stop(boost::bind(&DBusIPCAPI::CallbackWithStatus_Helper,
									 this, _1, message));
	ret = DBUS_HANDLER_RESULT_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_mfg_handler(
	NCPControlInterface* interface,
	DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_prop_get_handler(
	NCPControlInterface* interface,
	DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_prop_set_handler(
	NCPControlInterface* interface,
	DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_prop_insert_handler(
	NCPControlInterface* interface,
	DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_prop_remove_handler(
	NCPControlInterface* interface,
	DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_net_scan_start_handler(
	NCPControlInterface* interface,
	DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_discover_scan_start_handler(
	NCPControlInterface* interface,
	DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_energy_scan_start_handler(
	NCPControlInterface* interface,
	DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_pcap_to_fd_handler(
	NCPControlInterface* interface,
	DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_pcap_terminate_handler(
	NCPControlInterface* interface,
	DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_data_poll_handler(
	NCPControlInterface* interface,
	DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
	...
	interface->data_poll(boost::bind(&DBusIPCAPI::CallbackWithStatus_Helper,
								 this, _1, message));
	ret = DBUS_HANDLER_RESULT_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_config_gateway_handler(
	NCPControlInterface* interface,
	DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
	...
	if (valid_lifetime == 0) {
		interface->remove_on_mesh_prefix(
			...
			boost::bind(&DBusIPCAPI::CallbackWithStatus_Helper,this, _1, message)
		);
	} else {
...
DBusHandlerResult
DBusIPCAPI::service_add_handler(
	NCPControlInterface *interface,
	DBusMessage *message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::service_remove_handler(
	NCPControlInterface *interface,
	DBusMessage *message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_route_add_handler(
   NCPControlInterface* interface,
   DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_route_remove_handler(
   NCPControlInterface* interface,
   DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_joiner_attach_handler(
	NCPControlInterface* interface,
	DBusMessage *        message
) {
	ValueMap options;
...
DBusHandlerResult
DBusIPCAPI::interface_joiner_start_handler(
   NCPControlInterface* interface,
   DBusMessage *        message
) {
	ValueMap options;
...
DBusHandlerResult
DBusIPCAPI::interface_joiner_stop_handler(
   NCPControlInterface* interface,
   DBusMessage *        message
) {
	ValueMap options;
...
DBusHandlerResult
DBusIPCAPI::interface_joiner_commissioning_handler(
   NCPControlInterface* interface,
   DBusMessage *        message
) {
...
DBusHandlerResult
DBusIPCAPI::interface_joiner_add_handler(
   NCPControlInterface* interface,
   DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_joiner_remove_handler(
   NCPControlInterface* interface,
   DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_announce_begin_handler(
   NCPControlInterface* interface,
   DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_energy_scan_query_handler(
   NCPControlInterface* interface,
   DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_pan_id_query_handler(
   NCPControlInterface* interface,
   DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_generate_pskc(
   NCPControlInterface* interface,
   DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
	...
	interface->commissioner_generate_pskc(
		...
		network_name,
		xpan_id,
		boost::bind(&DBusIPCAPI::CallbackWithStatusArg1_Helper, this, _1, _2, message)
	);
...
DBusHandlerResult
DBusIPCAPI::interface_peek_handler(
   NCPControlInterface* interface,
   DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_poke_handler(
   NCPControlInterface* interface,
   DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_link_metrics_query_handler(
	NCPControlInterface* interface,
	DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_link_metrics_probe_handler(
	NCPControlInterface* interface,
	DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_link_metrics_mgmt_forward_handler(
	NCPControlInterface* interface,
	DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_link_metrics_mgmt_enh_ack_handler(
	NCPControlInterface* interface,
	DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_mlr_request_handler(
	NCPControlInterface* interface,
	DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
...
DBusHandlerResult
DBusIPCAPI::interface_backbone_router_config_handler(
	NCPControlInterface* interface,
	DBusMessage *        message
) {
	DBusHandlerResult ret = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
    ...
    )
{
	...
	if ((dbus_message_get_type(message) == DBUS_MESSAGE_TYPE_METHOD_CALL)
		&& (dbus_message_has_interface(message, WPAN_TUNNEL_DBUS_INTERFACE)
			|| dbus_message_has_interface(message, WPANTUND_DBUS_NLAPI_INTERFACE))
		...
		try {
			...
			DBusIPCAPI::CallbackWithStatus_Helper(kWPANTUNDStatus_InvalidArgument,message);
			ret = DBUS_HANDLER_RESULT_HANDLED;
		}
	}
...
DBusIPCAPI::dbus_message_handler(
	...
	std::pair<NCPControlInterface*,
	          DBusIPCAPI*> *cb_data =
	    (std::pair<NCPControlInterface*, DBusIPCAPI*> *)user_data;
