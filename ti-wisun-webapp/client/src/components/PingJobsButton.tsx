...
function PingJobsPortal(props: PingJobsPortalProps) {
  return ReactDOM.createPortal(<PingJobMonitor {...props} />, document.body);
}

export function PingJobsButton(props: PingJobsButtonProps) {
  const [shown, setShown] = useState(false);
  const theme = useContext(ThemeContext);
  const closePingJobs = useCallback(() => {
    setShown(false);
  }, [setShown]);
  const {fgColor} = pingJobsButtonThemeImplementations.get(theme);
  return (
    <>
      <svg
        style={{
      ...
      {shown && <PingJobsPortal closePingJobs={closePingJobs} {...props} />}
    </>
  );
}
